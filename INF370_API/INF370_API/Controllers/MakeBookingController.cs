...
namespace INF370_API.Controllers
{
    ...
    public class MakeBookingController : ApiController
    {
        [HttpGet]
        [Route("Slots/{ID}")]
        public List<dynamic> GetAvailableSlots(int ID)
        {

            INF370Entities db = new INF370Entities();
            db.Configuration.ProxyCreationEnabled = false;
            var prop = db.PROPERTies.Find(ID);
            return GetAvailableSlots(db.EMPLOYEEDATETIMESLOTs.Include(zz => zz.DATETIMESLOT).Include(zz => zz.EMPLOYEE).Where(zz => zz.EMPLOYEESLOTSTAUSID == 1 && zz.DATETIMESLOT.AREAID == prop.AREAID).ToList());
